...
```bash
uv sync
# set SECRET_KEY, SYSTEM_USERNAME, GROW_API_KEY, GOOGLE_OAUTH_CLIENT_ID, ADMIN_GOOGLE_SUB, DATABASE_URL (see README.md)
...
- **Types:** `uv run mypy grow` — imports `grow/settings.py` directly (not `tests.settings`), so it needs real env vars even though it's read-only: `SECRET_KEY`, `SYSTEM_USERNAME`, `GROW_API_KEY`, `DATABASE_URL`, `APP_VERSION`, `REDIS_URL`, `GOOGLE_OAUTH_CLIENT_ID`, `ADMIN_GOOGLE_SUB` all dummy values are fine, e.g.:
  ```bash
  SECRET_KEY=x SYSTEM_USERNAME=x GROW_API_KEY=x DATABASE_URL=sqlite:///:memory: APP_VERSION=0.0.0 REDIS_URL=redis://localhost:6379/0 GOOGLE_OAUTH_CLIENT_ID=x ADMIN_GOOGLE_SUB=x uv run mypy grow
  ```
...
- **Branching:** `main` (prod, Coolify auto-deploy) and `develop` (staging, Coolify auto-deploy) are both protected — no direct commits, ever. `feature/<name>` covers all develop-targeted work, branched from `develop`, merged back via PR. `release/<name>` cuts a release from `develop` into `main`. `hotfix/<name>` patches `main` directly for urgent fixes.
