...
- **Types:** `uv run mypy grow` — imports `grow/settings.py` directly (not `tests.settings`), so it needs real env vars even though it's read-only: `SECRET_KEY`, `SYSTEM_USERNAME`, `GROW_API_KEY`, `DATABASE_URL`, `REDIS_URL`, `GOOGLE_OAUTH_CLIENT_ID`, `ADMIN_GOOGLE_SUB` all dummy values are fine, e.g.:
  ```bash
  SECRET_KEY=x SYSTEM_USERNAME=x GROW_API_KEY=x DATABASE_URL=sqlite:///:memory: REDIS_URL=redis://localhost:6379/0 GOOGLE_OAUTH_CLIENT_ID=x ADMIN_GOOGLE_SUB=x uv run mypy grow
  ```
