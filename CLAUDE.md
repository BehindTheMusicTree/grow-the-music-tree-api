...
- **Types:** `uv run mypy grow` — imports `grow/settings.py` directly (not `tests.settings`), so it needs real env vars even though it's read-only: `SECRET_KEY`, `SYSTEM_USERNAME`, `GROW_API_KEY`, `DATABASE_URL`, `APP_VERSION`, `REDIS_URL` all dummy values are fine, e.g.:
  ```bash
  SECRET_KEY=x SYSTEM_USERNAME=x GROW_API_KEY=x DATABASE_URL=sqlite:///:memory: APP_VERSION=0.0.0 REDIS_URL=redis://localhost:6379/0 uv run mypy grow
  ```
