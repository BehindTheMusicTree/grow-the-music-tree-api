...
- **Google sign-in for the admin**: requests can authenticate with `Authorization: Bearer <Google ID token>`, verified against `GOOGLE_OAUTH_CLIENT_ID`. The account whose `sub` matches `ADMIN_GOOGLE_SUB` gets the `admin` role and can write; any other verified Google account is a read-only `viewer` (writes return 403 `permission_denied`). Invalid or expired tokens return 401 `invalid_token`. Both env vars are required.
- `GET /v{N}/auth/me/` returns the caller's `{"role", "email"}`, or 401 `authentication_required` when anonymous.
